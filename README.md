<div align="center">

# 💠 DHARSAN KAMARAJ

**`Software Developer · AI & ML Engineer · Embedded Systems Engineer`**

*SRM IST · CGPA 9.55 · Smart India Hackathon 2025 National Champion*

<!-- ROTATING ACHIEVEMENTS -->
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=14&duration=2600&pause=1000&color=64FFDA&center=true&vCenter=true&multiline=false&width=800&height=30&lines=SIH+2025+National+Winner+%7C+Prize+Rs.1%2C50%2C000;Deep+Learning+%7C+Computer+Vision+%7C+Embedded+AI;Ministry+of+Defence+Problem+Solver;Published+Researcher+in+XAI+%26+Defence+AI;Building+Systems+That+Operate+Beyond+Assumptions" alt="Highlights" />

<br/>

[![Gmail](https://img.shields.io/badge/Gmail-dharsan11032006-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:dharsan11032006@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-dharsan--kamaraj-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/dharsan-kamaraj)
[![GitHub](https://img.shields.io/badge/GitHub-KDHARSAN-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/KDHARSAN)
[![Location](https://img.shields.io/badge/📍-Trichy%2C%20India-FF6B6B?style=flat-square)](https://github.com/KDHARSAN)

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=KDHARSAN&style=flat-square&color=64FFDA&label=Profile+Views)
![Followers](https://img.shields.io/github/followers/KDHARSAN?style=flat-square&label=Followers&color=64FFDA)

</div>

---

## 🎯 Who Am I?

```python
class DharsanKamaraj:
    def __init__(self):
        self.name       = "Dharsan Kamaraj"
        self.role       = "Software Developer | AI/ML Engineer | Embedded Systems Builder"
        self.education  = "B.Tech CSE @ SRM IST, Trichy | CGPA: 9.55"
        self.languages  = ["Python", "Java", "JavaScript", "C++", "SQL", "Kotlin"]
        self.domains    = ["Machine Learning", "Computer Vision", "Embedded AI", "Full Stack"]
        self.superpower = "Turning Ministry of Defence problems into working prototypes 🛡️"

    def current_focus(self):
        return [
            "🛰️  Resilient GNSS-denied navigation using LEO satellite signals",
            "👁️  Medical AI — Glaucoma detection with ResAtt-UNet (98.38% accuracy)",
            "🔐  Explainable AI for critical infrastructure defence",
            "☕  Java Full Stack: Spring Boot + REST APIs + MySQL",
        ]

    def fun_facts(self):
        return [
            "🏆 National Winner — Smart India Hackathon 2025 (₹1,50,000 prize)",
            "📜 Published research paper presented internationally",
            "🇯🇵 Learning Japanese (JLPT N4 appearing)",
            "🔭 I make hardware talk to satellites using $20 SDR dongles",
        ]

me = DharsanKamaraj()
print("Hire me? Yes. Absolutely yes.")
```

---

## 🏆 Achievements

<div align="center">

### 🥇 Smart India Hackathon 2025 — National Champion

![Rank](https://img.shields.io/badge/Rank-1st%20Place-FFD700?style=flat-square)
![Prize](https://img.shields.io/badge/Prize-₹1%2C50%2C000-00D084?style=flat-square)
![Client](https://img.shields.io/badge/Client-Ministry%20of%20Defence-DC143C?style=flat-square)
![Theme](https://img.shields.io/badge/Theme-Space%20Technology-9B59B6?style=flat-square)

> Built a **Cognitive Opportunistic LEO-PNT Receiver** for the Defence Space Agency, HQ IDS —
> solving GNSS jamming, spoofing & signal attenuation in denied environments.

<br/>

<table>
<tr>
<td align="center" width="50%">

**🥇 AI in Action: Beyond the Horizon**
National Inter-College Paper Presentation
![Prize](https://img.shields.io/badge/1st%20Place-₹5%2C000-FFD700?style=flat-square)
*"Glaucoma Detection & Segmentation in Fundus Images Using UNet"*

</td>
<td align="center" width="50%">

**📄 Research Paper Published — 2025**
British University Vietnam (BUV)
![Status](https://img.shields.io/badge/Status-Published-64FFDA?style=flat-square)
*"Accountable XAI Framework for Real-Time Critical Infrastructure Defence"*

</td>
</tr>
</table>

<br/>

![Hackathons](https://img.shields.io/badge/Hackathons%20Won-2-FFD700?style=flat-square)
![Prize Total](https://img.shields.io/badge/Total%20Prize%20Money-₹1%2C55%2C000-00D084?style=flat-square)
![Papers](https://img.shields.io/badge/Papers%20Published-1-64FFDA?style=flat-square)
![CGPA](https://img.shields.io/badge/CGPA-9.55%2F10-9B59B6?style=flat-square)

</div>

---

## ⚙️ Tech Arsenal

<div align="center">

**🧠 AI / ML / Computer Vision**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)
![Keras](https://img.shields.io/badge/Keras-D00000?style=flat-square&logo=keras&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat-square&logo=python&logoColor=white)
![Roboflow](https://img.shields.io/badge/Roboflow-6706CE?style=flat-square)

**🖥️ Programming & Full Stack**

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat-square&logo=kotlin&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![REST API](https://img.shields.io/badge/REST%20API-FF6B6B?style=flat-square)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

**🔌 Embedded & Hardware**

![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-A22846?style=flat-square&logo=raspberrypi&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat-square&logo=arduino&logoColor=white)
![RTL-SDR](https://img.shields.io/badge/RTL--SDR-Signal%20Processing-64FFDA?style=flat-square)
![UART](https://img.shields.io/badge/UART%20%7C%20I²C%20%7C%20SPI-Protocols-blue?style=flat-square)
![IMU](https://img.shields.io/badge/IMU%20%7C%20GPS%20%7C%20Barometer-Sensors-orange?style=flat-square)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)

**🛠️ Tools**

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white)
![Google Colab](https://img.shields.io/badge/Google%20Colab-F9AB00?style=flat-square&logo=googlecolab&logoColor=black)

</div>

---

## 🚀 Flagship Projects

### 🛰️ 01 — Cognitive Opportunistic LEO-PNT Receiver

```
CLIENT  : Ministry of Defence — Defence Space Agency, HQ IDS
STACK   : Python | Raspberry Pi | RTL-SDR | Signal Processing
OUTCOME : Smart India Hackathon 2025 — National WINNER (₹1,50,000)
STATUS  : Functional prototype deployed & demonstrated nationally
```

**Problem:** GNSS systems are vulnerable to jamming, spoofing & signal attenuation.
**Solution:** Cognitive receiver using non-PNT LEO satellite signals as alternative navigation anchors.

- 🔭 RF front-end + SDR signal processing chain on Raspberry Pi
- 📡 Doppler shift estimation & timing synchronization modules
- 🧭 System-level compensation models for GNSS-denied environments
- ⚡ Real-time embedded prototype — full hardware–software integration

---

### 👁️ 02 — Deep Residual Attention U-Net for Glaucoma Detection

```
DOMAIN  : Medical AI | Computer Vision | Image Segmentation
STACK   : Python | TensorFlow | Keras | Deep Learning
OUTCOME : 1st Place — National AI Paper Presentation + Published Research
```

| Metric | Result |
|---|---|
| Segmentation Accuracy | **98.38%** |
| Mean IoU | **93.12%** |
| Improvement over U-Net | **+4.6%** |
| Model Parameters | **34M** |

- 🧠 ResAtt-UNet from scratch — residual blocks + attention gates
- 📈 Augmentation pipeline doubled dataset diversity
- ⚙️ Adam optimizer + binary cross-entropy for stable convergence

---

### 🔐 03 — XAI Framework for Critical Infrastructure Defence

```
TYPE      : Review Paper | Published Research
PUBLISHED : 2025 — British University Vietnam (BUV)
SCOPE     : International-level presentation
```

- Transparency & accountability in AI-driven security systems
- XAI methodologies for defence-oriented architectures
- Real-time risk mitigation & critical infrastructure threat modeling

---

## 💼 Experience

```
Jul–Sep 2025  Java Full Stack Intern — AICTE EduSkills
              Spring Boot | REST APIs | MySQL | HTML/CSS/JS | Git | Full SDLC

Apr–Sep 2024  Administrator Intern — Skill Academia  (6 months)
              Technical operations | System administration | Project documentation

Jun–Jul 2024  Machine Learning Intern — Internship Studio  (1 month)
              Python | Neural Networks | Feature Engineering | Hyperparameter Optimization
```

---

## 📊 GitHub Stats

<div align="center">

<table>
<tr>
<td>
<img src="https://github-readme-stats.vercel.app/api?username=KDHARSAN&show_icons=true&theme=tokyonight&border_color=64FFDA&title_color=64FFDA&icon_color=64FFDA&text_color=CCD6F6&bg_color=0D1117&include_all_commits=true&count_private=true&hide_border=false" width="340" />
</td>
<td>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=KDHARSAN&layout=compact&theme=tokyonight&border_color=64FFDA&title_color=64FFDA&text_color=CCD6F6&bg_color=0D1117&langs_count=6" width="260" />
</td>
</tr>
</table>

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=KDHARSAN&theme=tokyonight&border=64FFDA&ring=64FFDA&fire=FF6B6B&currStreakLabel=64FFDA&background=0D1117&sideLabels=CCD6F6&dates=8892B6" width="480" />

<br/><br/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=KDHARSAN&theme=tokyo-night&bg_color=0D1117&color=64FFDA&line=64FFDA&point=FF6B6B&area=true&hide_border=false&border_color=64FFDA" width="680" />

<br/><br/>

<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=KDHARSAN&theme=tokyonight" width="680" />

</div>

---

## ⚡ LeetCode

<div align="center">

<img src="https://leetcard.jacoblin.cool/rjsCHA28mn?theme=dark&font=Fira+Code&ext=activity&border=1&radius=6" width="460" />

</div>

---

## 🎓 Education

| Institution | Degree | Period | Score |
|---|---|---|---|
| **SRM IST, Trichy** | B.Tech — Computer Science & Engineering | 2023–2027 | **CGPA: 9.55** |
| Vidhyasagar International Public School | HSC | 2022–2023 | 82% |
| Mount Litera Zee School | SSLC | 2020–2021 | 72% |

---

## 🏅 Certifications

| Issuer | Certificate | Year |
|---|---|---|
| **Google** | Introduction to Generative AI | 2024 |
| **IBM** | Introduction to Artificial Intelligence | 2025 |
| **IBM** | Databases & SQL for Data Science — **WITH HONORS** | 2025 |
| **Amazon** | Data Structures and Algorithms | 2025 |
| **Northwestern University** | Fundamentals of Digital Image & Video Processing | 2025 |
| **Skill Academia** | UI Designing | 2024 |

---

## 🌐 Languages

![Tamil](https://img.shields.io/badge/Tamil-Native-FF6B6B?style=flat-square)
![English](https://img.shields.io/badge/English-Fluent-64FFDA?style=flat-square)
![Japanese](https://img.shields.io/badge/Japanese-N4%20Appearing-FF0000?style=flat-square)

---

## 🔭 Why Hire Me?

| | Capability |
|---|---|
| 🛡️ | Solved **real Ministry of Defence problems** — not toy datasets |
| 🛰️ | Built a **live satellite signal receiver** prototype on $20 hardware |
| 🧠 | Designed a **34M-parameter medical AI model** with 98.38% accuracy |
| 📄 | **Published internationally** — still in 3rd year of undergrad |
| 🏆 | **National hackathon champion** — proven under pressure |
| ⚙️ | Bridges **AI research and embedded systems** |
| 🔬 | Goes from **idea → hardware prototype → deployed system** end-to-end |

---

<div align="center">

*"Engineering systems that operate beyond assumptions."*

<br/>

[![Email](https://img.shields.io/badge/Email-dharsan11032006%40gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:dharsan11032006@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/dharsan-kamaraj)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/KDHARSAN)

<br/>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:112240,100:0A192F&height=100&section=footer&text=Open%20to%20Opportunities&fontSize=16&fontColor=64FFDA&animation=fadeIn&fontAlignY=65" />

</div>
